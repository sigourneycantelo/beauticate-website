...
var ThemeEditorBindings = function() {
  this.editing = true;
...
ThemeEditorBindings.prototype = {
  ...
  // Returns the first section in sectionOrder *after* sectionId that exists
  // in the iframe DOM. Multi-section ADD batches dispatch one addSection
  // message per new section; siblings further along in sectionOrder may not
  // be rendered yet when this runs, so a single-step lookup at index+1
  // would miss them and strand the section at the end via the append
  // fallback. Walking forward finds whichever pre-existing element comes
  // next, which is the correct anchor regardless of which siblings have
  // landed in the DOM so far.
  findNextSection: function(sectionId, sectionOrder) {
    var index = sectionOrder.findIndex(function(s) {
      return s === sectionId;
    });
    for (var i = index + 1; i < sectionOrder.length; i++) {
      var next = this.section(sectionOrder[i]);
  ...
  addSection: function(e, data) {
    ...
    $.post("", params, function(response) {
      // $.parseHTML is jQuery's documented anti-XSS API for converting an
      // HTML string into DOM nodes — it avoids parse-time script execution.
      // keepScripts=true retains <script> children so they still evaluate
      // when jQuery's domManip inserts the section into the live DOM
      // (matches the prior $(response) behavior for theme sections that
      // include legitimate embed/analytics scripts). Trust boundary: the
      // response comes from the same-origin Rails theme-preview endpoint.
      var section = $($.parseHTML(response, true));
      // Resolve the anchor at insert time, not dispatch time. Multi-section
      // ADD batches start parallel XHRs; resolving here ensures each insert
      // sees siblings that landed during this XHR's flight, so the final
      // DOM order matches sectionOrder regardless of XHR completion order.
      var insertBefore = editor.findNextSection(data.id, data.sectionOrder || []);
...
// CommonJS export for Jest. No-op in the asset-pipeline browser context
// where this file is loaded via a classic <script> tag.
if (typeof module !== "undefined" && module.exports) {
  module.exports = ThemeEditorBindings;
}
;
(function() {
